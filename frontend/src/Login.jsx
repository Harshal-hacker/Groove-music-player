...
import { Globe, Apple, ArrowLeft, Headphones, Loader2, Eye, EyeOff } from 'lucide-react';
        ...
        <div style={{ 
          ...
          {step === 1 && (
            <div style={{ animation: 'ultraFade 0.4s ease' }}>
              ...
              <form onSubmit={handleEmailNext} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div>
                  <label style={{ fontSize: '13px', fontWeight: '800', marginBottom: '8px', display: 'block', color: '#94a3b8' }}>Email</label>
                  <input 
                    type="text" placeholder="name@domain.com" required 
                    value={formData.email} 
                    onChange={(e) => setFormData({ ...formData, email: e.target.value })} 
                    style={{ 
                      width: '100%', padding: '16px 20px', background: '#0a0a0a', 
                      border: emailError ? '1px solid #ef4444' : '1px solid #333', 
                      borderRadius: '12px', color: '#fff', outline: 'none', fontSize: '14px', transition: '0.3s ease' 
                    }} 
                  />
                  {emailError && <div style={{ color: '#ef4444', fontSize: '12px', marginTop: '6px', fontWeight: '600' }}>{emailError}</div>}
                </div>
                ...
                <button 
                  ...
                  style={{ 
                    ...
                    border: 'none', fontWeight: '900', cursor: formData.email ? 'pointer' : 'not-allowed', 
                    fontSize: '15px', marginTop: '4px', opacity: formData.email ? 1 : 0.5, transition: '0.3s'
                  }}
                  onMouseOver={(e) => { if(formData.email) e.currentTarget.style.transform = 'translateY(-2px)' }}
                  onMouseOut={(e) => { if(formData.email) e.currentTarget.style.transform = 'translateY(0)' }}
                >
                  Next
                </button>
              </form>
            ...
            <div style={{ animation: 'slideInRight 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275)' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 18px', background: '#181818', border: '1px solid #333', borderRadius: '12px', marginBottom: '20px' }}>
                ...
                <button 
                  type="button" 
                  onClick={() => setStep(1)} 
                  style={{ background: 'none', border: 'none', color: '#10b981', fontWeight: '800', fontSize: '13px', cursor: 'pointer', padding: 0 }}
                >
                  Edit
                </button>
              </div>
              ...
              <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <button 
                  ...
                  style={{ 
                    padding: '18px', borderRadius: '50px', background: '#10b981', color: '#000', 
                    border: 'none', fontWeight: '900', cursor: formData.password ? 'pointer' : 'not-allowed', 
                    fontSize: '15px', marginTop: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center',
                    opacity: formData.password ? 1 : 0.5, transition: '0.3s'
                  }}
                  onMouseOver={(e) => { if(formData.password && !isSubmitting) e.currentTarget.style.transform = 'translateY(-2px)' }}
                  onMouseOut={(e) => { if(formData.password && !isSubmitting) e.currentTarget.style.transform = 'translateY(0)' }}
                >
                ...
                <button 
                  ...
                  style={{ 
                    ...
                    fontSize: '14px', marginTop: '4px', transition: '0.3s'
                  }}
